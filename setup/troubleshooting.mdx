...
<Step title="Enable Features Globally">
  ...
  2. Enable the features you want to offer (Chat, Writer, Imagine, etc.)
...
**Cause:**
Before v5.0, the title generator trimmed the content it read too aggressively and nudged some models toward English, so short or non-English content often got a title in the wrong language. v5.0 fixes both: titles are written in the language of the content, and fall back to the user's language only when the content is too short to tell.

**Solution:**

<Steps>
<Step title="Update to v5.0 or later">
  See [How to Update](/versioning/how-to-update).
</Step>
<Step title="Choose a capable title model">
  Some models follow language instructions better than others, which matters most for non-English content.

  1. Navigate to **Settings > Features > Utilities** in the admin panel
  2. Under **Title generator**, choose a different **Model**
  3. Click "Save changes" and test title generation again

  <Note>
  Since v5.0 the title generator model is one setting for the whole installation. Plans no longer override it.
  </Note>
</Step>
</Steps>
