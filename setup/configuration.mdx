...
<Card title="General Settings Guide" icon="settings" href="/website-basics/essentials">
...
A few feature settings worth reviewing:

- **Chat → Capabilities**: turn on **Media generation** (images, video, voice-overs and transcripts from chat), **Canvas** (documents in a side panel) and **Clarifying questions** (the model asks multiple-choice questions before answering an ambiguous request). Each capability must also be enabled on your plans.
- **Writer**: turns on the template wizard, which asks a template's questions and continues in chat. Manage the templates under **Templates**. See [Prompt Templates](/advanced/prompt-templates).
- **Community**: lets users share Imagine and Video generations to a feed visible to everyone on your installation. **Minimum shared generations** keeps the Community tab hidden until a tool has that many shared generations.
- **Utilities**: turns **Prompt enhancement** and **Surprise me** on or off for the Imagine and Video composers, and sets the model they use. It also sets the **Title generator** model for the whole installation.

To import the templates that ship with Aikeedo, go to **Settings > Starter content**. See [Starter Content](/advanced/seed-data). To choose which languages your users can pick, go to **Settings > Languages**. See [Localization](/advanced/localization).
...
<CardGroup cols={2}>
<Card title="Stripe" icon="brand-stripe" href="/integrations/payment-gateways/stripe">
  Set up Stripe for card payments and subscriptions
</Card>

<Card title="PayPal" icon="brand-paypal" href="/integrations/payment-gateways/paypal">
...
<Card title="Email Configuration" icon="mail" href="/email/mailer">
