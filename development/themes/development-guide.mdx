...
You can also work on a theme's translations from an Aikeedo installation, which
is the only way to fill them in automatically:
...
<Tip>
  Point `--path` at your theme's own repository rather than the built copy under
  `extra/`. See [Localization](/advanced/localization#themes-and-plugins).
</Tip>
