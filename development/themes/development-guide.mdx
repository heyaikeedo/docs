...
description: Learn how to create and customize themes for your Aikeedo platform using the modern themes-starter kit with Vite.js, Alpine.js, and Tailwind CSS.
---

import Support from '/snippets/support.mdx';

# Building Themes for Aikeedo v5.x
...
<Note>
This guide is for Aikeedo v5.x. We strongly recommend using the [themes-starter kit](https://github.com/heyaikeedo/themes-starter) for new theme development as it provides a modern build system and best practices.
</Note>
...
A plain `__()` in a theme template is looked up in Aikeedo's own catalog, not
the theme's, and is left out when the theme's strings are extracted.

Catalogs live in `static/locale/{language}/LC_MESSAGES/theme.po`, and the dev
server reloads the page when one changes. The starter kit has no extraction
script of its own: strings are extracted and translated with Aikeedo's locale
commands, run from your Aikeedo installation and pointed at the theme's
repository:

```bash
php bin/console app:locale:extract --path=../my-theme
php bin/console app:locale:translate --path=../my-theme --all
```

Run the extract command again whenever you add or change wording.

<Tip>
  Point `--path` at your theme's repository root, not the built copy under
  `extra/`: the next build or update replaces that copy. See
  [Localization](/advanced/localization#themes-and-plugins).
</Tip>
