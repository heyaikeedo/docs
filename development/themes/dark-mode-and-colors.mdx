...
<Check>The mode is applied in the head, before the body renders.</Check>
<Check>Only the configured modes are reachable, and the switcher hides when there's one.</Check>
<Check>Both modes are checked for contrast, including borders and dimmed text.</Check>
<Check>Images and logos have dark variants where they need them, for example `option.brand.logo_dark`.</Check>
<Check>The choice persists across page loads.</Check>
