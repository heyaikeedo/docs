...
<ResponseField name="plan-config" type="Plan Config Object">
  <Expandable title="Properties">
    ...
    <ResponseField name="chat" type="object">
      ...
      <ResponseField name="custom_assistants_cap" type="integer|null">
        Maximum number of custom assistants a workspace can create. `null` means no limit.
      </ResponseField>
      <ResponseField name="temporary_chat" type="boolean">
        Indicates whether users can start temporary chats.
      </ResponseField>
    </ResponseField>
    ...
    <ResponseField name="video" type="object">
      <ResponseField name="is_enabled" type="boolean">
        Indicates whether the video feature is enabled for this plan.
      </ResponseField>
    </ResponseField>

    <ResponseField name="models" type="object">
      An object containing boolean flags keyed by model, indicating which models are available in this plan. If set to `null`, all enabled models are available. Examples include:
      - `gpt-5.6-sol`, `gpt-5.5`, `gpt-4o-mini`
      - `claude-opus-5`, `claude-sonnet-5`, `claude-haiku-4-5`
      - `grok-4.6`, `command-a-plus-05-2026`
      - `gpt-image-2`, `fal-ai/nano-banana-2`, `bytedance/seedance-2.5`
      - `tts-1`, `eleven_multilingual_v2`, `whisper-1`
    </ResponseField>

    <ResponseField name="tools" type="object">
      An object specifying which tools are available in this plan:
      - `embedding_search`: Boolean indicating if file analysis is enabled
      - `google_search`: Boolean indicating if Google search is enabled
      - `youtube`: Boolean indicating if YouTube metadata lookup is enabled
      - `web_scrap`: Boolean indicating if web browsing is enabled
      - `generate_media`: Boolean indicating if image, video, voice-over and transcript generation from chat is enabled
      - `memory`: Boolean indicating if memory is enabled
      - `canvas`: Boolean indicating if Canvas is enabled
      - `ask_questions`: Boolean indicating if clarifying questions are enabled
    </ResponseField>
    ...
    <ResponseField name="presets" type="array<string>|null">
      An array of template (preset) IDs that are available in this plan, covering Writer, image and video templates. If set to null, all templates are available.
    </ResponseField>

    <ResponseField name="extensions" type="object">
      Plan settings contributed by installed plugins, keyed by plugin.
    </ResponseField>
  </Expandable>
</ResponseField>
