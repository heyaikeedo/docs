...
<Check>The installed directory matches `name`.</Check>
<Check>`type` is `aikeedo-theme`.</Check>
<Check>`require` includes `heyaikeedo/composer`.</Check>
<Check>`version` is set and raised for the release.</Check>
<Check>`extra.public` lists everything the browser needs.</Check>
<Check>If there's PHP, the entry class and the PSR-4 namespace agree.</Check>
