...
<Check>`version` in `static/composer.json` is raised.</Check>
<Check>`extra.public` lists `assets` and `.vite`, so the built files reach the web root.</Check>
<Check>Translation catalogs are extracted and up to date.</Check>
<Check>The entry class namespace matches the PSR-4 map, if the theme ships PHP.</Check>
<Check>`THEME_ASSETS_SERVER` is unset in the installation you tested against, so you tested the built assets.</Check>
<Check>Nothing private is in `static/`: no `.env`, keys or customer data.</Check>
