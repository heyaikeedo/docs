...
| `…\Admin\AbstractAdminViewRequestHandler` | `/admin` | + DemoEnvironment, License, View | Admin pages rendered with Twig |
...
<Warning>
  A handler that extends no base class gets no authentication, no body parsing and no error handling. Only do that when you need it, such as for an embeddable widget, and add middleware explicitly. See [Public endpoints and embeds](/development/plugins/public-endpoints-and-embeds).
</Warning>
