...
<Check>The directory path matches `name`.</Check>
<Check>`type` is `aikeedo-plugin`.</Check>
<Check>`require` includes `heyaikeedo/composer`.</Check>
<Check>`extra.entry-class` points at a class that implements `PluginInterface`.</Check>
<Check>`autoload.psr-4` maps that namespace to `src/`.</Check>
<Check>`version` is set and increases with each release.</Check>
