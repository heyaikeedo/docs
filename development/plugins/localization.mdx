...
<Check>Every user-visible string goes through `__()`, `p__()` or `n__()`.</Check>
<Check>Navigation labels use the `nav` context.</Check>
<Check>Catalogs live at `locale/{code}/LC_MESSAGES/messages.po`.</Check>
<Check>Every catalog sets `X-Domain: messages`.</Check>
<Check>You re-ran extraction after the last wording change.</Check>
<Check>The `locale/` directory is in your release archive.</Check>
