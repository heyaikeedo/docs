...
```bash
php bin/console app:make:module
```

The generator scaffolds a module with the layout above: entity, value objects, repository interface and implementation, CRUD commands and handlers, and a bootstrapper. It's the fastest way to see the conventions applied consistently.

<Warning>
  A generated module lives in `src/`, which an update replaces. Use it to learn the structure, or when you maintain a fork; plugins can't add modules. See [Data and persistence](/development/plugins/data-and-persistence).
</Warning>
