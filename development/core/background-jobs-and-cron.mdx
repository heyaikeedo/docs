...
| Backfill message parts | Migrates older message data |
...
<Check>Process a fixed number of records per run.</Check>
<Check>Persist a cursor, so the next run continues.</Check>
<Check>Keep a status, so an administrator can start, pause and stop the work.</Check>
<Check>Return early when there's nothing to do, so an idle installation costs nothing.</Check>
