...
<Tip>
  Most of the providers below ship as ready-made servers with their current
  models. Import them from **Settings → Starter content → LLM servers**, then add
  your API key. See [Starter content](/advanced/seed-data#llm-servers-and-their-models).
</Tip>
...
**Example for Azure OpenAI Service**:
```text
https://my-instance.openai.azure.com/openai/deployments/{model}
```
When using model "gpt-4o-mini", the actual request will go to:
```text
https://my-instance.openai.azure.com/openai/deployments/gpt-4o-mini
```

<Tip>
  Dynamic variables are useful for services where the model name is part of the API endpoint. Most providers take the model name in the request body instead and don't need them.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Hugging Face">
    ...
    Use the model's Hugging Face ID as the model key, for example `deepseek-ai/DeepSeek-V4-Pro-0813`. Hugging Face routes the request to one of its inference providers.

    <Warning>
      The old `https://api-inference.huggingface.co/models/{model}/v1/` address no longer works. If you added Hugging Face with it, change the server address to the one above.
    </Warning>
  </Accordion>
  ...
  <Accordion title="Nebius">
    ...
    <Note>
      Nebius AI Studio is now Nebius Token Factory. Servers added with the old `https://api.studio.nebius.ai/v1/` address still respond, but switch to the address above.
    </Note>
  </Accordion>
...
2. **Writer**: Template-driven writing that continues in chat
3. **Title Generation**: Automatic title creation for content
4. **Prompt Enhancement**: Rewriting and generating prompts in the Imagine and Video composers
