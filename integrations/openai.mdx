...
<Warning>
  **BYOK is not available for features that require OpenAI webhooks.** Video
  generation always uses the system-wide API key because it depends on webhook
  notifications for proper functionality.
</Warning>
...
1. **Chat**: Engage in intelligent conversations, with Canvas documents and clarifying questions.
2. **Writer**: Answer a template's questions and continue the result in chat.
3. **Imagine**: Create and edit images from text descriptions and reference images.
4. **Transcriber**: Convert audio to text with high accuracy.
5. **Voiceover**: Transform text into natural-sounding speech.
6. **Classifier**: Categorize and analyze text content.
7. **Utilities**: Generate titles and enhance prompts.
