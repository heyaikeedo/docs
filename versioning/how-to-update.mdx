...
<Warning>
  <b>Upgrading from v2.x to v3.0?</b> This guide is <u>not</u> suitable for
  major version upgrades. To upgrade from v2.x to v3.0, you must follow the
  dedicated{' '}
  <a href="/versioning/release-notes#migration-guide-upgrading-from-v2-x-to-v3-0">
    Migration Guide
  </a>{' '}
  instead. Attempting to use the standard update process may result in data loss
  or a broken installation.
</Warning>

<Note>
  The migration guide is required for all major version upgrades (e.g., v2.x to
  v3.0). Use this update guide only for minor or patch updates within the same
  major version.
</Note>
